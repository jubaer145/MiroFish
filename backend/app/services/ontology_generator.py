...
**Important: You must output valid JSON format data only, with no other content.**
...
- We need to simulate reactions from all parties in a public opinion event and information propagation paths
...
```json
{
    ...
    "analysis_summary": "Brief analysis summary of the text content (in the language of the input)"
}
```
...
A. **Fallback types (required, placed last in the list as the final 2)**:
   - `Person`: The fallback type for any individual person. When a person does not belong to any other more specific person type, classify them here.
   - `Organization`: The fallback type for any organizational entity. When an organization does not belong to any other more specific organization type, classify it here.

B. **Specific types (8 types, designed based on text content)**:
   - Designed for the main character types appearing in the text
...
- Various characters appear in text, such as "primary school teachers", "bystanders", "random netizens"
- Without a dedicated type match, they should be classified under `Person`
...
- Person: Any individual person (used when no specific type matches)
...
- Organization: Any organizational entity (used when no specific type matches)
...
class OntologyGenerator:
    ...
    def generate(
        ...
        # Call LLM (use 8192 tokens — Gemini tends to be verbose)
